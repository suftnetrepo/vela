...
import {
  ...
  StyledHeader, StyledPressable, StyledDivider, StyledTextInput, Switch,
} from 'fluent-styles'
...
function PrefRow({
  icon, label, subtitle, onPress, right, iconBg, destructive = false,
}: {
  icon:         VelaIconName
  label:        string
  subtitle?:    string
  ...
  return (
    <Stack gap={0}>
      <StyledText fontSize={12} fontWeight="700" color={Colors.textTertiary}
        letterSpacing={0.5} paddingHorizontal={4} paddingBottom={8}>
        {title}
      </StyledText>
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const tu = await settingsService.get<TempUnit>(SETTINGS_KEYS.TEMPERATURE_UNIT)
      const fd = await settingsService.get<FirstDay>(SETTINGS_KEYS.FIRST_DAY_OF_WEEK)
  ...
  const handleDeleteData = async () => {
    const ok = await dialogueService.confirm({
      ...
      message:      'This will permanently delete all your cycle logs, symptoms, and settings. This cannot be undone.',
      icon:         '⚠️',
    ...
    if (!ok) return
    toastService.info('Feature coming soon', 'Data export & deletion will be in the next update.')
  }

  return (
    <StyledPage flex={1} backgroundColor={Colors.background}>
      <StyledHeader title="Account" titleAlignment="left" showStatusBar showBackArrow
      ...
      <StyledScrollView contentContainerStyle={{ padding: 20, paddingBottom: 48, gap: 20 }}
        ...
        {/* Profile hero card */}
        <Stack backgroundColor={Colors.surface} borderRadius={24} padding={20}
          shadowColor="#000" shadowOffset={{ width: 0, height: 2 }}
          shadowOpacity={0.07} shadowRadius={12} elevation={3}>
          <Stack horizontal alignItems="center" gap={16}>
            <Stack width={64} height={64} borderRadius={32} backgroundColor={Colors.primaryFaint}
            ...
            <Stack flex={1} gap={4}>
              ...
              {isPremium ? (
                <Stack horizontal alignItems="center" gap={5} backgroundColor={Colors.primaryFaint}
                  borderRadius={10} paddingHorizontal={10} paddingVertical={4} alignSelf="flex-start">
                ...
                <StyledPressable onPress={() => router.push('/(app)/(settings)/premium')}
                  flexDirection="row" alignItems="center" gap={5} backgroundColor={Colors.primaryFaint}
                  borderRadius={10} paddingHorizontal={10} paddingVertical={4} alignSelf="flex-start">
                  <VelaIcon name="crown" size={12} color={Colors.primary} />
        ...
        <SectionCard title="PREFERENCES">
          <PrefRow icon="weight" label="Weight unit"
            subtitle={weightUnit === 'kg' ? 'Kilograms (kg)' : 'Pounds (lbs)'}
            right={
              <StyledPressable onPress={() => cycleUnit(weightUnit, ['kg', 'lbs'], 'weight_unit', setWeightUnit)}
          ...
          <PrefRow icon="thermometer" label="Temperature unit"
            subtitle={tempUnit === 'celsius' ? 'Celsius (°C)' : 'Fahrenheit (°F)'}
            right={
              <StyledPressable
                onPress={() => cycleUnit(tempUnit, ['celsius', 'fahrenheit'], SETTINGS_KEYS.TEMPERATURE_UNIT, setTempUnit)}
                backgroundColor={Colors.primaryFaint} borderRadius={10}
                paddingHorizontal={12} paddingVertical={6}>
                <StyledText fontSize={13} fontWeight="700" color={Colors.primaryDark}>
          ...
          />
          <StyledDivider borderBottomColor={Colors.border} marginHorizontal={16} />
          <PrefRow icon="calendar" label="First day of week"
            subtitle={firstDay === 'monday' ? 'Monday' : 'Sunday'}
            right={
              <StyledPressable
                onPress={() => cycleUnit(firstDay, ['monday', 'sunday'], SETTINGS_KEYS.FIRST_DAY_OF_WEEK, setFirstDay)}
                backgroundColor={Colors.primaryFaint} borderRadius={10}
                paddingHorizontal={12} paddingVertical={6}>
                <StyledText fontSize={13} fontWeight="700" color={Colors.primaryDark}>
          ...
          />
          <StyledDivider borderBottomColor={Colors.border} marginHorizontal={16} />
          <PrefRow icon="activity" label="Body Mass Index"
            subtitle="Track BMI alongside weight"
            right={
              <Switch value={bmiEnabled}
                onChange={v => { setBmiEnabled(v); toggle('bmi_enabled', v) }}
                activeColor={Colors.primary} inactiveColor={Colors.border} size="sm" />
            }
          />
        </SectionCard>
        ...
        <SectionCard title="ACCOUNT & SECURITY">
          <PrefRow icon="lock" label="PIN & Biometrics"
            subtitle="Protect access to Vela"
            onPress={() => router.push('/(app)/(settings)/security')} />
          <StyledDivider borderBottomColor={Colors.border} marginHorizontal={16} />
          <PrefRow icon="palette" label="Appearance"
        ...
        <SectionCard title="DATA & PRIVACY">
          <PrefRow icon="shield-check" label="Privacy policy"
            onPress={() => router.push('/(app)/(settings)/privacy')} />
          <StyledDivider borderBottomColor={Colors.border} marginHorizontal={16} />
          <PrefRow icon="download" label="Export my data"
            subtitle="Coming soon — Premium feature"
            onPress={() => toastService.info('Coming soon', 'Data export will be available in a future update.')} />
          <StyledDivider borderBottomColor={Colors.border} marginHorizontal={16} />
          <PrefRow icon="trash" label="Clear all data"
        ...
        <SectionCard title="HELP">
          <PrefRow icon="help" label="FAQ"
            onPress={() => router.push('/(app)/(settings)/faq')} />
          <StyledDivider borderBottomColor={Colors.border} marginHorizontal={16} />
          <PrefRow icon="info" label="About Vela"
            subtitle="Version 1.0.0" />
        </SectionCard>

        {/* Footer */}
        <Stack alignItems="center" gap={4} paddingTop={4}>
          <Stack width={40} height={40} borderRadius={20} backgroundColor={Colors.primaryFaint}
