...
import {
  ...
  StyledHeader, StyledPressable, StyledTextInput,
} from 'fluent-styles'
...
export default function FAQScreen() {
  ...
  return (
    <StyledPage flex={1} backgroundColor={Colors.background}>
      <StyledHeader title="FAQ" titleAlignment="center" showBackArrow
      ...
      <StyledScrollView contentContainerStyle={{ paddingBottom: 48 }}
        ...
        {/* Hero */}
        <Stack alignItems="center" paddingHorizontal={24} paddingTop={8} paddingBottom={20} gap={8}>
        ...
        {/* Category chips */}
        <Stack horizontal paddingHorizontal={20} paddingBottom={20} gap={8} flexWrap="wrap">
          <StyledPressable
            onPress={() => setActiveCategory(null)}
            backgroundColor={!activeCategory ? Colors.primary : Colors.surfaceAlt}
        ...
        <Stack paddingHorizontal={20} gap={8}>
          ...
          {filtered.map((item, i) => {
            ...
            return (
              <Stack key={key} backgroundColor={Colors.surface} borderRadius={18} overflow="hidden"
                shadowColor="#000" shadowOffset={{ width: 0, height: 1 }}
                shadowOpacity={0.05} shadowRadius={6} elevation={1}>
                <StyledPressable onPress={() => setExpanded(isOpen ? null : key)}
                  flexDirection="row" alignItems="center" padding={16} gap={12}>
                  <Stack width={32} height={32} borderRadius={10} backgroundColor={Colors.primaryFaint}
                    alignItems="center" justifyContent="center">
                    <VelaIcon name={isOpen ? 'minus' : 'plus'} size={16} color={Colors.primary} />
                  </Stack>
                  <StyledText fontSize={14} fontWeight="700" color={Colors.textPrimary} flex={1}
                    lineHeight={20}>
                    {item.q}
                  </StyledText>
                </StyledPressable>
                {isOpen && (
                  <Stack paddingHorizontal={16} paddingBottom={16} paddingLeft={60}>
        ...
        {/* Still need help */}
        <Stack margin={20} backgroundColor={Colors.primaryFaint} borderRadius={20} padding={20}
          ...
          <StyledText fontSize={13} color={Colors.textSecondary} lineHeight={20}>
            Since Vela is fully offline, we can't offer in-app chat support. But you can reach us via the App Store review, or find us on GitHub.
          </StyledText>
          <Stack backgroundColor={Colors.surface} borderRadius={12} padding={12}>
            <StyledText fontSize={12} color={Colors.textTertiary} fontWeight="600">
