...
import {
  Stack, StyledText, StyledScrollView, StyledPage, StyledHeader, StyledPressable,
} from 'fluent-styles'
...
const ARTICLE_CONTENT: Record<string, {
  title: string; subtitle: string; readTime: string; category: string; content: string[]
}> = {
  'menstrual-cycle': {
    title: 'Understanding Your Menstrual Cycle', subtitle: 'A complete guide to the four phases',
    readTime: '5 min read', category: 'Basics',
    content: [
  ...
  'fertile-window': {
    title: 'Your Fertile Window Explained', subtitle: 'When are you most likely to conceive',
    readTime: '4 min read', category: 'Fertility',
    content: [
  ...
  'bbt-tracking': {
    title: 'Basal Body Temperature Tracking', subtitle: 'How to use BBT to understand your cycle',
    readTime: '4 min read', category: 'Tracking',
    content: [
  ...
  'pms-symptoms': {
    title: 'PMS: Symptoms & Management', subtitle: 'Understanding premenstrual syndrome',
    readTime: '5 min read', category: 'Wellness',
    content: [
  ...
  'irregular-cycles': {
    title: 'Irregular Cycles: What\'s Normal?', subtitle: 'Understanding cycle variation',
    readTime: '4 min read', category: 'Health',
    content: [
...
export default function ArticleScreen() {
  ...
  if (!article) {
    return (
      <StyledPage flex={1} backgroundColor={Colors.background}>
        <StyledHeader title="Article" showBackArrow onBackPress={() => router.push('/(app)/(settings)/articles')}
          showStatusBar backgroundColor={Colors.background} />
        <Stack flex={1} alignItems="center" justifyContent="center">
          <StyledText color={Colors.textSecondary}>Article not found.</StyledText>
        </Stack>
      </StyledPage>
    )
  }
  ...
  return (
    <StyledPage flex={1} backgroundColor={Colors.background}>
      <StyledHeader title="" showBackArrow onBackPress={() => router.push('/(app)/(settings)/articles')}
        showStatusBar backgroundColor={Colors.background} />
      ...
      <StyledScrollView contentContainerStyle={{ paddingBottom: 60 }}
        ...
        {/* Header */}
        <Stack paddingHorizontal={24} paddingTop={4} paddingBottom={28} gap={14}>
          <Stack horizontal alignItems="center" gap={8}>
            <Stack backgroundColor={accentColor + '20'} borderRadius={10}
              paddingHorizontal={10} paddingVertical={5}>
              <StyledText fontSize={11} fontWeight="700" color={accentColor}>
                {article.category.toUpperCase()}
              </StyledText>
            </Stack>
            <StyledText fontSize={12} color={Colors.textTertiary}>{article.readTime}</StyledText>
          </Stack>

          <StyledText fontSize={24} fontWeight="800" color={Colors.textPrimary} lineHeight={32}>
            {article.title}
          </StyledText>
          <StyledText fontSize={15} color={Colors.textSecondary} lineHeight={23}>
            {article.subtitle}
          </StyledText>
                ...
                {parts.map((part, j) => {
                  ...
                  return (
                    <StyledText
                      ...
                      color={isHeading ? Colors.textPrimary : Colors.textSecondary}
        ...
        {/* Footer disclaimer */}
        <Stack margin={24} backgroundColor={Colors.warningLight} borderRadius={16} padding={16} gap={6}>
          ...
          <StyledText fontSize={12} color={Colors.textSecondary} lineHeight={18}>
            This article is for general education only and is not medical advice. Always consult a qualified healthcare provider for personal medical questions.
          </StyledText>
        </Stack>
      </StyledScrollView>
    </StyledPage>
