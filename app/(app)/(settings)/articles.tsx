...
const ARTICLES: Omit<Article, 'iconBg' | 'iconColor'>[] = [
  {
    ...
    subtitle: 'A complete guide to the four phases and what happens in your body',
    readTime: '5 min read',
    icon: 'cycle',
    category: 'Basics',
    content: [
...
function ArticleCard({ article, onPress }: { article: typeof ARTICLES[0]; onPress: () => void }) {
  ...
  return (
    <StyledPressable onPress={onPress} backgroundColor={Colors.surface} borderRadius={20}
      padding={16} shadowColor="#000" shadowOffset={{ width: 0, height: 2 }}
      shadowOpacity={0.06} shadowRadius={10} elevation={2} flexDirection="row" gap={14}>
      <Stack width={52} height={52} borderRadius={16} backgroundColor={iconBgs[article.category] ?? Colors.primaryFaint}
        alignItems="center" justifyContent="center" flexShrink={0}>
        <VelaIcon name={article.icon} size={26} color={iconColors[article.category] ?? Colors.primary} />
      </Stack>
      <Stack flex={1} gap={5}>
        <Stack horizontal alignItems="center" gap={6}>
          <Stack backgroundColor={iconBgs[article.category] ?? Colors.primaryFaint} borderRadius={8}
            paddingHorizontal={8} paddingVertical={3}>
            <StyledText fontSize={10} fontWeight="700" color={iconColors[article.category] ?? Colors.primary}>
              {article.category.toUpperCase()}
            </StyledText>
          </Stack>
          <StyledText fontSize={11} color={Colors.textTertiary}>{article.readTime}</StyledText>
        </Stack>
        <StyledText fontSize={14} fontWeight="700" color={Colors.textPrimary} lineHeight={20}>
          {article.title}
        </StyledText>
        <StyledText fontSize={12} color={Colors.textSecondary} lineHeight={17} numberOfLines={2}>
          {article.subtitle}
        </StyledText>
      </Stack>
...
export default function ArticlesScreen() {
  ...
  return (
    <StyledPage flex={1} backgroundColor={Colors.background}>
      ...
      <StyledScrollView contentContainerStyle={{ padding: 20, paddingBottom: 48, gap: 12 }}
        showsVerticalScrollIndicator={false}>

        {/* Hero */}
        <Stack backgroundColor={Colors.primaryFaint} borderRadius={20} padding={20} gap={8}
          ...
          <StyledText fontSize={13} color={Colors.textSecondary} lineHeight={20}>
            Evidence-based articles written to help you understand your cycle, fertility, and health. All content is stored offline.
          </StyledText>
        </Stack>
